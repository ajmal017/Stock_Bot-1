...
STARTING_BALANCE = 35000  # starting equity
...
def buy_stock():
    ...
    if side == 'long entry':
        ...
        try:
            hook.send("```diff" + "\n" +
                      ...
                      "+ " + str('Current Available Buying Power: ' + '${:,.2f}'.format(float(account.buying_power) - float(total_price))) + "\n" +
    ...
    # EXIT
    # EXIT
    if side == 'short take profit' or side == 'short stop loss' or side == 'long take profit' or side == 'long stop loss' or side == 'exit timer':
        ...
        try:
            hook.send("```diff" + "\n" +
                      ...
                      "- " + "Price: " + '$' + str(symbol_price) + "\n" +
                      "- " + "Quantity: " + response['qty'] + "\n" +
                      "- " + str('Total Price: ' + '$+{:,.2f}'.format(float(symbol_price) * float(response['qty']))) + "\n" +
                      "- " + str('Current Available Funds: ' + '${:,.2f}'.format(float(account.equity))) + "\n" +
                      "- " + str('Current Available Buying Power: ' + '${:,.2f}'.format(float(account.buying_power) + float(total_price))) + "\n" +
                      "- " + str('Today\'s Profit: ' + '${:,.2f}'.format(float(balance_change)) + " (" + '{:.2f}%'.format(float(today_percentage)) + ")") + "\n" +
                      "- " + str('Total Profit: ' + '${:,.2f}'.format(float(total_profit)) + " (" + '{:.2f}%'.format(float(total_percentage)) + ")") + "\n" +
